...
const STATUS_CONFIG: Record<StatusAparelho, { label: string; icon: string; color: string; bgColor: string; borderColor: string }> = {
  NOVO_OK: {
    label: 'Novo / OK',
    icon: 'check_circle',
...
export function CadastroIndividualPage() {
  ...
  const [tipo, setTipo] = useState<TipoAparelho>('RASTREADOR')
  ...
  const [status, setStatus] = useState<StatusAparelho>('NOVO_OK')
  const [categoriaFalha, setCategoriaFalha] = useState<CategoriaFalha>('FALHA_COMUNICACAO')
  const [destinoDefeito, setDestinoDefeito] = useState<DestinoDefeito>('LABORATORIO')
  ...
  const { data: aparelhosExistentes = [] } = useQuery<ExistingId[]>({
    ...
    select: (data) => data.filter((a: any) => a.identificador).map((a: any) => ({
  ...
  const idJaExiste = useMemo(() => {
    ...
    return found || null
  }, [identificador, aparelhosExistentes])

  const tecnicosFiltrados = useMemo(() => {
    if (!buscaResponsavel.trim()) return tecnicos.slice(0, 10)
    return tecnicos.filter((t) =>
      t.nome.toLowerCase().includes(buscaResponsavel.toLowerCase())
    ).slice(0, 10)
  ...
  const podeSalvar = useMemo(() => {
    if (!identificador.trim() || !idValido) return false
    if (tipo === 'RASTREADOR' && (!marca || !modelo)) return false
    if (tipo === 'SIM' && !operadora) return false
    if (origem === 'DEVOLUCAO_TECNICO' && !tecnicoSelecionadoId) return false
    if (origem !== 'DEVOLUCAO_TECNICO' && !responsavelEntrega.trim()) return false
    return true
  }, [
    identificador,
    idValido,
  ...
  const limparFormulario = useCallback((manterConfiguracao: boolean = false) => {
    setIdentificador('')
    if (!manterConfiguracao) {
      setTipo('RASTREADOR')
      setMarca('')
      setModelo('')
      setOperadora('')
      ...
      setBuscaResponsavel('')
      setTecnicoSelecionadoId(null)
      setObservacoes('')
      setStatus('NOVO_OK')
      setCategoriaFalha('FALHA_COMUNICACAO')
      setDestinoDefeito('LABORATORIO')
    }
  }, [])
    ...
    mutationFn: async () => {
      ...
      const payload = {
        ...
        responsavelEntrega: origem === 'DEVOLUCAO_TECNICO' 
          ...
          : responsavelEntrega,
        tecnicoId: origem === 'DEVOLUCAO_TECNICO' ? tecnicoSelecionadoId : null,
        observacoes: observacoes || null,
        statusEntrada: status,
        categoriaFalha: status === 'CANCELADO_DEFEITO' ? categoriaFalha : null,
        destinoDefeito: status === 'CANCELADO_DEFEITO' ? destinoDefeito : null,
      }
  ...
  const handleCadastrarEFinalizar = () => {
    createAparelhoMutation.mutate(undefined, {
      onSuccess: () => {
        navigate('/aparelhos')
      },
    })
  ...
  const statusRevisao = useMemo(() => {
    ...
    if (!idValido && identificador.trim()) return 'ID_INVALIDO'
    ...
    <div className="-m-4 min-h-[100dvh] bg-slate-100">
        ...
        {quantidadeCadastrada > 0 && (
          <div className="flex items-center gap-2 bg-emerald-50 border border-emerald-200 rounded-sm px-4 py-2">
            ...
            <span className="text-sm font-bold text-emerald-800">
              {quantidadeCadastrada} equipamento{quantidadeCadastrada > 1 ? 's' : ''} cadastrado{quantidadeCadastrada > 1 ? 's' : ''} nesta sessão
            </span>
          </div>
        )}
      </header>
        ...
        <div className="flex-1 space-y-6">
          ...
          <div className="bg-white border border-slate-200 rounded-sm p-6">
            <div className="flex items-center gap-2 mb-6 pb-2 border-b border-slate-100">
              <MaterialIcon name="qr_code_scanner" className="text-blue-600" />
              <h3 className="text-xs font-bold text-slate-800 uppercase tracking-wide">Identificação Técnica</h3>
            </div>
            <div className="grid grid-cols-2 gap-6">
              {/* IMEI / ICCID */}
              <div className="col-span-2">
                <Label className="text-[10px] font-bold text-slate-500 uppercase mb-1.5 block">
                  {tipo === 'RASTREADOR' ? 'IMEI' : 'ICCID'} / Número de Série <span className="text-red-500">*</span>
                </Label>
                <div className="relative">
                  <Input
                    ...
                    className={cn(
                      'h-10 pr-10 font-mono',
                      idJaExiste && 'border-red-500 bg-red-50 focus:ring-red-500 focus:border-red-500',
                      !idJaExiste && idValido && identificador.trim() && 'border-emerald-500 bg-emerald-50 focus:ring-emerald-500 focus:border-emerald-500'
                    )}
                  />
                  {idJaExiste && (
                    <MaterialIcon name="error" className="absolute right-3 top-2.5 text-red-500" />
                  )}
                  {!idJaExiste && idValido && identificador.trim() && (
                ...
                {idJaExiste && (
                  <p className="text-[10px] text-red-600 mt-1 font-bold uppercase">
                    Atenção: Este {tipo === 'RASTREADOR' ? 'IMEI' : 'ICCID'} já consta no sistema
                ...
                {!idJaExiste && !idValido && identificador.trim() && (
                  <p className="text-[10px] text-amber-600 mt-1 font-bold uppercase">
                    O {tipo === 'RASTREADOR' ? 'IMEI deve ter 15 dígitos' : 'ICCID deve ter 19-20 dígitos'}
                  </p>
                )}
              </div>
              ...
              {/* Tipo de Equipamento */}
              <div>
                <Label className="text-[10px] font-bold text-slate-500 uppercase mb-1.5 block">Tipo de Equipamento</Label>
                <Select value={tipo} onValueChange={(v) => setTipo(v as TipoAparelho)}>
                ...
                <div>
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    ...
                    <Select value={modelo} onValueChange={setModelo} disabled={!marca}>
                      <SelectTrigger className="h-10">
                        <SelectValue placeholder={marca ? 'Modelo...' : 'Selecione marca'} />
                      </SelectTrigger>
                      <SelectContent>
          ...
          <div className="bg-white border border-slate-200 rounded-sm p-6">
            <div className="flex items-center gap-2 mb-6 pb-2 border-b border-slate-100">
              <MaterialIcon name="history_edu" className="text-blue-600" />
              <h3 className="text-xs font-bold text-slate-800 uppercase tracking-wide">Origem e Rastreabilidade</h3>
            </div>
            <div className="grid grid-cols-2 gap-6">
              {/* Origem do Item */}
              <div>
                ...
                <Select value={origem} onValueChange={(v) => setOrigem(v as OrigemItem)}>
                  <SelectTrigger className="h-10">
                    <SelectValue />
                  </SelectTrigger>
                  <SelectContent>
              ...
              <div>
                ...
                {origem === 'DEVOLUCAO_TECNICO' ? (
                  <div>
                    ...
                    {tecnicosFiltrados.length > 0 && buscaResponsavel && !tecnicoSelecionadoId && (
                      <div className="mt-2 border border-slate-200 rounded-sm max-h-40 overflow-y-auto">
                        {tecnicosFiltrados.map((tecnico) => (
                          <button
                            ...
                            type="button"
                            onClick={() => {
                              setTecnicoSelecionadoId(tecnico.id)
                              setBuscaResponsavel('')
                            }}
              ...
              <div className="col-span-2">
                ...
                <textarea
                  ...
                  className="w-full h-20 p-3 bg-slate-50 border border-slate-300 rounded-sm text-sm focus:bg-white focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all resize-none"
                />
              </div>
            </div>
          </div>
          ...
          <div className="bg-white border border-slate-200 rounded-sm p-6">
            <div className="flex items-center gap-2 mb-6 pb-2 border-b border-slate-100">
              <MaterialIcon name="settings_suggest" className="text-blue-600" />
              <h3 className="text-xs font-bold text-slate-800 uppercase tracking-wide">Definição de Status</h3>
            </div>
              ...
              {status === 'CANCELADO_DEFEITO' && (
                <div className="p-4 bg-red-50 border border-red-100 rounded-sm grid grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <Select value={categoriaFalha} onValueChange={(v) => setCategoriaFalha(v as CategoriaFalha)}>
                  ...
                  <div>
                    ...
                    <Select value={destinoDefeito} onValueChange={(v) => setDestinoDefeito(v as DestinoDefeito)}>
                      <SelectTrigger className="h-10 border-red-200">
                        <SelectValue />
                      </SelectTrigger>
                      <SelectContent>
            ...
            <div className="bg-slate-800 text-white rounded-sm shadow-xl overflow-hidden border border-slate-700">
              ...
              <div className="p-6 space-y-6">
                ...
                <div className="grid grid-cols-1 gap-4">
                  <div>
                    <label className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Identificador</label>
                    <p className={cn(
                      'text-sm font-mono',
                      idJaExiste ? 'text-red-400' : identificador.trim() ? 'text-white' : 'text-slate-500'
                    )}>
                      {identificador.trim() || '--- não definido ---'}
                    </p>
                  </div>
                  <div>
                    <label className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Responsável Origem</label>
                    <p className="text-sm font-medium">
                      {origem === 'DEVOLUCAO_TECNICO' 
                        ? (tecnicoSelecionado?.nome || '--- não definido ---')
                        : (responsavelEntrega.trim() || '--- não definido ---')
                      }
                    </p>
                  </div>
                </div>
                <div className="pt-4 border-t border-slate-700">
                  <div className="flex items-center justify-between mb-2">
                    <label className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Status Definido</label>
                    <span className={cn(
                      'text-[10px] font-bold px-2 py-0.5 rounded-sm uppercase tracking-tighter',
                      status === 'NOVO_OK' && 'bg-emerald-600/20 text-emerald-400 border border-emerald-600/50',
                      status === 'EM_MANUTENCAO' && 'bg-blue-600/20 text-blue-400 border border-blue-600/50',
                      status === 'CANCELADO_DEFEITO' && 'bg-red-600/20 text-red-400 border border-red-600/50'
                    )}>
                      {STATUS_CONFIG[status].label}
                    </span>
                  </div>
                  <div className="flex items-center justify-between">
                    <label className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Data Entrada</label>
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div className="space-y-1">
                    ...
                    <p className="text-[10px] text-slate-400 leading-tight">
                      Este registro será vinculado à sua conta e gerará um log de entrada individual para fins de rastreabilidade patrimonial.
                    </p>
                  </div>
                </div>
              </div>
              ...
              <div className="mt-4 p-4 bg-amber-50 border border-amber-200 rounded-sm">
                <div className="flex gap-3">
                  ...
                  <p className="text-[11px] text-amber-800 font-medium">
                    O {tipo === 'RASTREADOR' ? 'IMEI' : 'ICCID'} informado já possui um registro ativo. Registrar novamente criará um histórico de duplicidade.
                  </p>
                </div>
              </div>
            )}
      ...
      <div className="sticky -bottom-4 z-10 h-20 border-t border-slate-200 px-8 flex items-center justify-end gap-4 bg-white">
        ...
        <Button
          ...
          disabled={!podeSalvar || createAparelhoMutation.isPending}
          className="h-11 px-6 text-[11px] font-bold uppercase gap-2 border-blue-200 text-blue-600 hover:bg-blue-50"
        >
