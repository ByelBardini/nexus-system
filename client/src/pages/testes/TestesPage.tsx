...
export function TestesPage() {
  ...
  const aparelhoSelecionado = imeiSearch.trim()
    ? (rastreadores.find(
        (r) =>
          (r.identificador ?? "").toLowerCase() ===
          imeiSearch.trim().toLowerCase(),
      ) ?? null)
    : null;
  ...
  useEffect(() => {
    ...
    if (
      ...
      return;
    const match = rastreadores.find(
      (r) => (r.identificador ?? "").trim().toLowerCase() === id.toLowerCase(),
    );
    if (match) {
  ...
  return (
    <div className="-m-4 flex flex-1 min-h-0 overflow-hidden">
      ...
      <Dialog
        ...
        onOpenChange={(o) => !o && setShowRetiradaModal(false)}
      >
        <DialogContent
          hideClose
      ...
      <Dialog
        ...
        onOpenChange={(o) => !o && setShowCancelarModal(false)}
      >
        <DialogContent
          hideClose
          className="max-w-md p-0 gap-0 overflow-hidden rounded-sm"
