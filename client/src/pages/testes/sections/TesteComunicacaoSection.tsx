...
import type { ComunicacaoResult } from '../testes-types'
  ...
  return (
    <section className="bg-white border border-slate-300 shadow-sm overflow-hidden">
      <div className="bg-slate-50 border-b border-slate-300 px-4 py-2 flex items-center gap-2">
        <MaterialIcon name="wifi" className="text-erp-blue text-lg" />
        <h2 className="text-xs font-bold text-slate-700 font-condensed uppercase">
          03. Validação de Comunicação GPRS/GPS
        </h2>
      </div>
            ...
            return (
              <button
                ...
                className={cn(
                  ...
                  isActive ? opt.activeClasses : 'border-slate-200 bg-white hover:border-slate-300',
                  !isActive && opt.value === 'COMUNICANDO' && 'hover:border-green-500 hover:bg-green-50',
                  !isActive && opt.value === 'AGUARDANDO' && 'hover:border-amber-500 hover:bg-amber-50',
                  !isActive && opt.value === 'NAO_COMUNICOU' && 'hover:border-red-500 hover:bg-red-50'
                )}
              >
                <div
                  className={cn(
                    'w-12 h-12 rounded-full flex items-center justify-center mb-3 transition-colors',
                    isActive ? opt.iconClasses : cn('bg-slate-200', opt.iconHoverClasses)
                  )}
                >
          ...
          <div className="flex flex-wrap items-end gap-4">
            ...
            {onPosChaveChange && (
              <div>
                ...
                <button
                  type="button"
                  onClick={() => onPosChaveChange(posChave === 'SIM' ? 'NAO' : 'SIM')}
                  className={cn(
