...
Carrega até 500 registros de uma vez — sem paginação incremental no frontend. O mapeamento `mapDebitoApiToView` (linha 73) transforma `DebitoRastreadorApi` → `DebitoEquipamento` (view local).
