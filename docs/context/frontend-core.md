...
| `formatarDataDiaMesAno(iso)` | `string` | `"12/04/2026"` ou `"-"` se vazio | Só data numérica (dd/mm/aaaa); usa `parseDataLocal`; usado p.ex. na coluna “Últ. mov.” da página de débitos de equipamentos |
...
#### `cpf-cnpj-validation.ts`

Utilitários de validação de dígito verificador. Nenhuma dependência externa.

| Export | Comportamento |
|--------|--------------|
| `isCpfCnpjValidationEnabled()` | Retorna `false` **somente** se `VITE_VALIDATE_CPF_CNPJ === "false"`; qualquer outro valor (incluindo ausência) retorna `true` — padrão produção é habilitado |
| `validarCPF(digits)` | Mod-11 padrão; rejeita sequências repetidas (ex.: `111.111.111-11`) |
| `validarCNPJ(digits)` | Mod-11 padrão; rejeita sequências repetidas |
| `validarCPFouCNPJ(digits)` | Delega por tamanho: 11 dígitos → `validarCPF`, 14 → `validarCNPJ`, outros → `false` |

Usada pelos refinements Zod de `tecnicoFormSchema` (`cpfCnpj`), `clienteFormSchema` (`cnpj`) e `criacaoOsFormSchema` (`subclienteCpf`). Campos vazios/`undefined` sempre passam (campos opcionais). Para desabilitar em dev: `VITE_VALIDATE_CPF_CNPJ=false` no `.env`.
