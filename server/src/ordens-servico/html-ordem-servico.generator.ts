...
export class HtmlOrdemServicoGenerator {
  ...
  gerar(os: unknown): string {
    ...
    const isRevisaoOuRetirada = ['REVISAO', 'RETIRADA'].includes(d.tipo);
  ...
  <main class="a4-container flex flex-col">
    ...
    <section class="mb-3">
      <h3 class="label-title mb-1 border-l-4 border-blue-600 pl-2 flex items-center gap-2">
        ...
        Tipo de Serviço${isRevisaoOuRetirada ? ' e Detalhes' : ''}
      </h3>
      <div class="grid grid-cols-2 ${isRevisaoOuRetirada ? 'md:grid-cols-4' : ''} gap-x-6 gap-y-3 p-3 border border-gray-100 rounded">
        <div><label class="label-title block">Tipo</label><span class="field-value font-semibold">${esc(tipoLabel)}</span></div>
        ${isRevisaoOuRetirada ? `
        <div><label class="label-title block">${d.tipo === 'RETIRADA' ? 'ID a retirar' : 'ID a substituir'}</label><span class="field-value">${esc(d.idAparelho) || '-'}</span></div>
        <div><label class="label-title block">Local de instalação</label><span class="field-value">${esc(d.localInstalacao) || '-'}</span></div>
        <div><label class="label-title block">Pós-Chave</label><span class="field-value">${d.posChave === 'SIM' ? 'Sim' : d.posChave === 'NAO' ? 'Não' : '-'}</span></div>
        ` : ''}
      </div>
    </section>
    ...
    <section class="mb-3">
      ...
      <div class="p-3 border border-gray-100 rounded bg-gray-50/30">
        <p class="field-value">${esc(d.observacoes) || '-'}</p>
      </div>
    </section>
