...
describe('ClientesService', () => {
  ...
  describe('findAll', () => {
    it('retorna lista de clientes sem subclientes por padrão', async () => {
      const clientes = [{ id: 1, nome: 'Cliente A', contatos: [], _count: { ordensServico: 0 } }];
  ...
  describe('findOne', () => {
    ...
    it('retorna cliente quando encontrado', async () => {
      const cliente = { id: 1, nome: 'Cliente A', contatos: [], subclientes: [] };
  ...
  describe('create', () => {
    ...
    it('cria cliente com contatos', async () => {
      const dto = {
        nome: 'Novo Cliente',
        contatos: [{ nome: 'João', celular: '11999999999', email: 'joao@test.com' }],
      };
      const created = { id: 1, nome: 'Novo Cliente', contatos: [{ id: 1, nome: 'João' }] };
      prisma.cliente.create.mockResolvedValue(created);
  ...
  describe('update', () => {
    it('lança NotFoundException quando cliente não existe', async () => {
      ...
      await expect(service.update(999, { nome: 'Novo Nome' })).rejects.toThrow(NotFoundException);
    });
    ...
    it('atualiza contatos via transação quando contatos são informados', async () => {
      ...
      const updated = { id: 1, nome: 'Cliente A', contatos: [{ id: 1, nome: 'Novo' }] };
      prisma.cliente.update.mockResolvedValue(updated);

      await service.update(1, { contatos: [{ nome: 'Novo', celular: '11999' }] } as any);
